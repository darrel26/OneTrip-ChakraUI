...
import { storeDestinationDate, storeLocation, storeOriginsDate } from '../../../../Redux/ReduxSlices';
...
export default function HomePage() {
  ...
  const allowAddTrip = () =>{
    if (originsDate.current.value !== "" && destinationDate.current.value !== "" && location.current.value !== "") {
      setDisable("/addTrip")
    }else{
      setDisable("#")
    }
        ...
        <VStack w="full" h="full">
          ...
          >
            ...
            <HStack
              ...
              borderRadius="xl"
            >
              <InputLocation locationRef={location} _onChangeFunction={allowAddTrip} />
              <InputDate dateRef={originsDate} _onChangeFunction={allowAddTrip} />
              <InputDate dateRef={destinationDate} _onChangeFunction={allowAddTrip} />
              <Link to={isDisable}>
                <IconButton onClick={storeToRedux} Link colorScheme="teal" p={8} icon={<AddIcon />} />
              </Link>
            </HStack>
          </VStack>
        </VStack>
