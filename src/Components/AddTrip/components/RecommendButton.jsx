...
const Buttons = ({ title, onClickHandler }) => {
  return (
    <Button onClick={() => onClickHandler()} boxShadow="base">
      {title}
    </Button>
  );
};
...
const RecommendButton = () => {
  ...
  const getLoc = useSelector((state) => state.trip.basedLocation);
  const {
    isOpen: ismuseumOpen,
    onOpen: onmuseumOpen,
  ...
  const dispatcher = (restriction) => {
    dispatch(storeUserPreference(restriction));
